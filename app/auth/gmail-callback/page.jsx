  ...
  useEffect(() => {
    const processCallback = async () => {
      try {
        ...
        // Response handling with detailed error logging
        if (!tokenResponse.ok) {
          const errorText = await tokenResponse.text();
          console.log('Token exchange error details:', errorText);
          try {
        ...
        setStatus('Fetching Gmail information...');
        ...
        if (!userInfoResponse.ok) {
          throw new Error('Failed to fetch Gmail user information');
        }

        const userInfo = await userInfoResponse.json();
        const email = userInfo.email;

        // Verify the email matches the expected email (if specified)
        const expectedEmail = localStorage.getItem('gmail_auth_email');
        if (expectedEmail && email !== expectedEmail) {
          throw new Error(
            "The Gmail account you connected doesn't match your login email. Please use the same account."
          );
        }
        ...
        // Update the database with the Gmail refresh token
        ...
        setIsSuccess(true);
        toast.success(`Successfully connected Gmail: ${email}`);

        // Flag that Gmail auth was successful for the ConnectionsButton component
        localStorage.setItem('gmail_auth_success', 'true');
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Mail className="mr-2 h-10 w-10 text-red-500" />
          ...
          {isSuccess === null && (
            ...
            >
              ...
              >
                ...
                >
                  <Loader2 className="mr-4 h-12 w-12 text-red-500" />
                </motion.div>
...
function GmailCallbackLoading() {
  return (
    <div className="flex min-h-screen items-center justify-center">
      ...
      <p className="ml-4 text-lg">Loading Gmail callback...</p>
    </div>
  );
}
