...
export default function MediaComponent({
  ...
  const toggleVideoOption = (option) => {
    ...
    // Dispatch global event when video is unmuted to pause auto scroll in carousels
    if (option === 'muted' && typeof window !== 'undefined') {
      const event = new CustomEvent('videoMuteToggle', {
        detail: {
          isMuted: newOptions.muted,
          videoId: itemId,
          action: newOptions.muted ? 'muted' : 'unmuted',
        },
      });
        ...
        <div className="relative h-full w-full">
          ...
          {!isEditing && (
            <button
              ...
              className="absolute left-2 top-2 z-50 rounded-full bg-gradient-to-tr from-transparent via-gray-500 to-transparent p-1.5 text-white transition-colors"
    ...
    >
      ...
      {!isEditing && mediaLink && (
        <a
          ...
          target="_blank"
          onClick={(e) =>
            isTenant && trackButtonClick(e, user.username, card?.title)
          }
          rel="noopener noreferrer"
          className="absolute right-2 top-2 z-40 rounded-full bg-white p-1"
        >
          <CircleArrowOutUpRight
            onClick={(e) => {
              e.stopPropagation();
